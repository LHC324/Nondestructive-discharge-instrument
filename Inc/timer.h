...
typedef struct
{
	...
	unsigned char set_flag; /*定时器被设置标志：设置了定时器时间但是时间未到*/
	unsigned int count;		/*时间计数器*/
} soft_timer_t;

extern TIMERS Timer_Group[TIMER_EVENTS];
extern soft_timer_t soft_timer_group[TIMER_EVENTS];
extern void Timer0_Init();
extern void Timer1_Init();

#if (USIING_OTA)
#define _timer_ota soft_timer_group[0]
#endif
